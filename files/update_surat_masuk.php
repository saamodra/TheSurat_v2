...
if ($id=anti_injection($_REQUEST['id'])) {
    $exe = mysqli_query($con, "SELECT s.nomor_surat, b.bagian, s.sifat, s.perihal, s.tanggal_surat, s.tanggal_terima, s.tanggal_expired, s.nomor_agenda, s.lampiran, s.disposisi, s.tembusan, i.nama_instansi, j.jenis, s.tindak_lanjut, l.lokasi, l.keterangan, s.bagian_id, s.jenis_id, s.lokasi_id, s.instansi_id FROM surat_masuk s LEFT JOIN bagian b ON s.bagian_id=b.bagian_id LEFT JOIN instansi i ON s.instansi_id=i.instansi_id LEFT JOIN jenis j ON s.jenis_id=j.jenis_id LEFT JOIN lokasi l ON s.lokasi_id=l.lokasi_id WHERE s.surat_masuk_id ='$id'");
...
if (isset($_POST['update']) && ($id=anti_injection($_REQUEST['id']))) {
        ...
        if ($lampiran == "") {
            ...
            $hasil= mysqli_query($con, $update) or die (mysqli_error());
             mysqli_query($con2, 'insert into log(userid, menu, ip, log) values ("'.$id_user.'","Surat Masuk","'.$ip.'","'.$update.'")');
            write_log($update);
            ...
            <div class="row">
                ...
                <div class="row">
                    <div class="form-group col-md-12">
                        ...
                        <input type="date" value="<?php echo $row->tanggal_terima; ?>" class="form-control" name="tanggal_terima" >
                    </div>
                </div>
                <div class="row">
                    <div class="form-group col-md-12">
                        <label for="exampleInputTglExpired">Tanggal Expired</label>
                        <input type="date" value="<?php echo $row->tanggal_expired; ?>" class="form-control" name="tanggal_expired" >
                    </div>
                </div>
                <div class="row">
                    <div class="form-group col-md-12">
                ...
                <div class="row">
                    <div class="form-group col-md-12">
                        ...
                        <select name="tindak_lanjut" class="form-control" >
                            <option value="TIDAK PERLU TINDAK LANJUT" <?php if($row->tindak_lanjut=='TIDAK PERLU TINDAK LANJUT'){echo 'selected';}?>>TIDAK PERLU TINDAK LANJUT</option>
                            <option value="PERLU TINDAK LANJUT" <?php if($row->tindak_lanjut=='PERLU TINDAK LANJUT'){echo 'selected';}?>>PERLU TINDAK LANJUT</option>
                        </select>
                    </div>
                </div>
                <div class="row">
