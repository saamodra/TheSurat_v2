<?php
    ...
    $pdf->SetTitle("Data Block IP");
    $pdf->Ln(10);//Ln = pindah baris
    $pdf->SetFont('Arial','B',14);
    $pdf->Cell(190,10,'Data Block IP',0,0,'C');
    ...
    $pdf->SetXY($x + 10, $y);
    $pdf->MultiCell(90, 5,'IP ADDRESS','T','C');
    $pdf->SetXY($x + 100, $y);
    ...
    $pdf->Ln(5);
    $no = 1;
    $sql = "SELECT * FROM block_ip ORDER BY ip DESC";
    ...
    while($data = mysqli_fetch_array($query)){
      ...
      $pdf->MultiCell(10,5, $no.".",  'T', 'C');
      $pdf->SetXY($x + 10, $y);
      $pdf->MultiCell(90,5, $data["ip"], 'T', 'C');
      $pdf->SetXY($x + 100, $y);
    ...
    //cetak
    $pdf->Output("", "Data Block IP (".date("H-i-s")."_".date("d-m-Y").")");
    write_log('Export PDF Block IP ('.$_SESSION['user_id'].')');
    mysqli_query($con2, "insert into log(userid, menu, ip, log) values ('".$_SESSION['user_id']."','Block IP','$ip','Export PDF Block IP (".$_SESSION['user_id'].")')");
?>