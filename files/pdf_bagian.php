<?php
    ...
    $pdf->SetTitle("Data Bagian");
    $pdf->Ln(10);//Ln = pindah baris
    $pdf->SetFont('Arial','B',14);
    $pdf->Cell(190,10,'Data Bagian',0,0,'C');
    ...
    $pdf->Ln(5);
    $no = 1;
    $sql = "SELECT * FROM bagian ORDER BY bagian DESC";
    ...
    while($data = mysqli_fetch_array($query)){
      ...
      $pdf->MultiCell(10,5, $no.".",  'T', 'C');
      $pdf->SetXY($x + 10, $y);
      $pdf->MultiCell(90,5, $data["bagian"], 'T', 'C');
      $pdf->SetXY($x + 100, $y);
    ...
    //cetak
    $pdf->Output("", "Data Bagian (".date("H-i-s")."_".date("d-m-Y").")");
    write_log('Export PDF Bagian ('.$_SESSION['user_id'].')');
    mysqli_query($con2, "insert into log(userid, menu, ip, log) values ('".$_SESSION['user_id']."','Bagian','$ip','Export PDF Bagian (".$_SESSION['user_id'].")')");
?>