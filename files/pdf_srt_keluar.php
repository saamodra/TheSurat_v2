<?php
    ...
    $pdf->Cell(190,10,'Data Surat Keluar',0,0,'C');
    ...
    while($data = mysqli_fetch_array($query)){
      ...
      $pdf->SetXY($x + 110, $y);
      $pdf->MultiCell(25,5, $data['tanggal_kirim'],'T', 'C');
      $pdf->SetXY($x + 135, $y);
    ...
    //cetak
    $pdf->Output("", "Data Surat Keluar (".date("H-i-s")."_".date("d-m-Y").")");
    write_log('Export PDF Surat Keluar ('.$_SESSION['user_id'].')');
    mysqli_query($con2, "insert into log(userid, menu, ip, log) values ('".$_SESSION['user_id']."','Surat Keluar','$ip','Export PDF Surat Keluar (".$_SESSION['user_id'].")')");
?>